...
def build_argparser():
    ...
    args.add_argument("--input_file", help="Path to the input text file", required=False, type=str)
    args.add_argument("--output_file", help="Path to the output text file", required=False, type=str)
    return parser
...
def save_output_to_file(output_text, file_path):
...
def main():
    ...
    if args.input_file:
        input_text = load_input_from_file(args.input_file)
        log.info("Input text from file: {}".format(input_text))
        input_prompts = [input_text]
    elif args.input:
        input_prompts = args.input
    else:
        input_prompts = []
    ...
    for prompt in prompts():
        ...
        # save result to file if output file is specified
        if args.output_file:
            save_output_to_file(text, args.output_file)
            log.info("Output saved to file: {}".format(args.output_file))
