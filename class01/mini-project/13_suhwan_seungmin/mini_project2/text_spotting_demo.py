...
# 현재 스크립트와 동일한 디렉토리에 있는 common/python을 추가
common_python_path = current_file.parent / 'common/python'
sys.path.append(str(common_python_path))

# 현재 스크립트와 동일한 디렉토리에 있는 common/python/model_zoo를 추가
common_model_zoo_path = current_file.parent / 'common/python/model_zoo'
sys.path.append(str(common_model_zoo_path))
...
def sort_boxes(centers, texts):
            # 각 단어를 (텍스트, 좌표) 형태로 리스트로 만듭니다.
            text_boxes = list(zip(texts, centers))

            # y 좌표를 기준으로 먼저 정렬합니다.
            text_boxes.sort(key=lambda x: x[1][1])

            # 정렬된 리스트를 행 단위로 묶습니다.
            rows = []
            current_row = []
            current_y = text_boxes[0][1][1]
            for tb in text_boxes:
                text, box = tb
                if abs(box[1] - current_y) > 10:  # 같은 행에 있는지 판단하는 기준 (10은 임의의 값, 필요시 조정)
            ...
            # 각 행 내에서 x 좌표 기준으로 정렬합니다.
            for row in rows:
                row.sort(key=lambda x: x[1][0])

            # 최종적으로 정렬된 텍스트와 좌표 리스트를 만듭니다.
            sorted_text_boxes = [tb for row in rows for tb in row]
...
def main():
    ...
    while frame is not None:
        ...
        texts = []
        txt = []
        for feature, box in zip(text_features, boxes):
            ...
            for i in range(MAX_SEQ_LEN):
                ...
                prev_symbol_index = int(np.argmax(symbols_distr, axis=1)[0])
            ...
            texts.append(text if text_confidence >= args.tr_threshold else '') # 시각화를 위한 텍스트 쌓기

            if text_confidence >= args.tr_threshold:
                txt.append(text) # 정렬을 위한 텍스트 쌓기
                centers.append((box[0], box[1], box[2], box[3]))

        result_texts = sort_boxes(centers, txt) # 좌측 상단부터 한 줄 씩 정렬

        # 텍스트 파일로 저장
        with open('ocr_results.txt', 'w') as f:
            for text, _ in result_texts:
                f.write(text + "\n")
    ...
    # Define a function to sort bounding boxes
